...
function admin_print_custom_styles() {
	if ( function_exists( 'wp_enqueue_media' ) ) {
		...
		wp_enqueue_style( 'arvo', '//fonts.googleapis.com/css?family=Open+Sans:400,300,600,700|Arvo:400,400italic,700,700italic', '', MAINTENANCE_CACHE_TIME );
		wp_enqueue_style( 'wp-color-picker' );

		wp_enqueue_script( 'uplaods_', MAINTENANCE_URI . 'js/uploads_.min.js', 'jquery', MAINTENANCE_CACHE_TIME, true );
		wp_register_script( 'maintenance', MAINTENANCE_URI . 'js/init.js', array( 'wp-color-picker' ), MAINTENANCE_CACHE_TIME, true );
		wp_localize_script( 'maintenance', 'maintenance', array( 'path' => MAINTENANCE_URI ) );
		wp_enqueue_script( 'maintenance' );
		wp_enqueue_style( 'maintenance', MAINTENANCE_URI . 'css/admin.css', '', MAINTENANCE_CACHE_TIME );
}
...
function load_later_scripts() {
	...
	if ( ! empty( $current_screen->id ) && $current_screen->id === 'toplevel_page_maintenance' ) {
		...
		wp_enqueue_script( 'select2', MAINTENANCE_URI . 'js/select2/select2.min.js', 'jquery', MAINTENANCE_CACHE_TIME, true );
		wp_enqueue_style( 'select2', MAINTENANCE_URI . 'js/select2/select2.css', '', MAINTENANCE_CACHE_TIME );
	}
}
